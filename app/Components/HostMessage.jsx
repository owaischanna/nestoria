...
import {
    Search, Plus, Phone, Paperclip, Send, MoreVertical,
    CheckCheck, X, Loader2, Image as ImageIcon, Smile, Menu, ArrowLeft
} from 'lucide-react';
import HostSidebar from "@/app/Components/HostSidebar";
import HostHeader from "./HostHeader";
...
const ConversationListItem = ({ conversation, isActive, onClick, currentUserId }) => {
    ...
    const isUnread = lastMessage && !isHostSender && lastMessage.sender !== currentUserId;
    ...
    const AvatarComponent = () => {
        ...
        return (
            <div className="relative">
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-gradient-to-br from-green-400 to-emerald-600 text-white flex items-center justify-center text-sm md:text-base font-bold shadow-md">
                    {initials}
                </div>
                <div className="absolute bottom-0 right-0 w-2.5 h-2.5 md:w-3.5 md:h-3.5 bg-green-500 rounded-full border-2 border-white"></div>
            </div>
        );
    };
    ...
    return (
        <div
            onClick={() => onClick(conversation._id)}
            className={`flex p-3 md:p-4 rounded-2xl cursor-pointer transition-all duration-200 ${
                isActive 
                    ? 'bg-gradient-to-r from-green-50 to-emerald-50 shadow-md scale-[1.02]' 
                    : 'hover:bg-gray-50 hover:shadow-sm'
            }`}
        >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-1">
                    <p className={`font-bold text-gray-900 truncate text-sm md:text-base ${unreadCount > 0 ? 'text-green-900' : ''}`}>
                        {senderName}
                    </p>
                    <span className="text-xs text-gray-500 ml-2 flex-shrink-0">{messageTime}</span>
                </div>
                <p className={`text-xs md:text-sm truncate mb-2 ${unreadCount > 0 ? 'text-gray-800 font-medium' : 'text-gray-600'}`}>
                    {previewText}
                </p>
                <div className="flex items-center justify-between">
                    <span className={`text-xs font-bold px-2 py-1 md:px-3 md:py-1 rounded-full capitalize ${
                        participantRole === 'host' 
                            ? 'bg-gradient-to-r from-orange-100 to-amber-100 text-orange-800' 
                            : 'bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-800'
                    }`}>
                    ...
                    {unreadCount > 0 && (
                        <span className="text-xs font-bold min-w-[18px] h-4 md:h-5 flex items-center justify-center rounded-full bg-gradient-to-r from-red-500 to-pink-500 text-white px-1 md:px-2 shadow-md">
                            {unreadCount}
                        </span>
                    )}
                </div>
    ...
    return (
        <div className={`flex ${alignment} w-full mb-4 group`}>
            <div className={`flex items-end space-x-2 max-w-[85%] md:max-w-[70%] ${isCurrentUser ? 'flex-row-reverse space-x-reverse' : ''}`}>
                {!isCurrentUser && (
                    <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-gradient-to-br from-gray-300 to-gray-400 flex-shrink-0 mb-1"></div>
                )}
                <div className="flex flex-col">
                    <div className={`rounded-2xl px-3 py-2 md:px-4 md:py-3 shadow-md ${
                        isCurrentUser 
                            ? 'bg-gradient-to-br from-green-500 to-emerald-600 text-white rounded-br-md' 
                            : 'bg-white text-gray-800 rounded-bl-md border border-gray-100'
                    }`}>
                        <p className="text-sm md:text-[15px] leading-relaxed break-words">{message.content}</p>
                    </div>
                    <div className={`flex items-center space-x-1 mt-1 px-1 md:px-2 ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
                        <span className={`text-xs ${isCurrentUser ? 'text-gray-500' : 'text-gray-500'}`}>
                            {time}
                        </span>
                        {isCurrentUser && (
                            <CheckCheck className="w-3 h-3 md:w-3.5 md:h-3.5 text-green-600" />
                        )}
                    </div>
                </div>
            </div>
        ...
        <div className="flex h-screen bg-gray-50 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <main className="flex-1 flex overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
                    ...
                    {/* Sidebar */}
                    <div className={`w-full lg:w-[380px] flex-shrink-0 border-r border-gray-200 bg-white shadow-xl flex flex-col fixed lg:static inset-y-0 left-0 z-40 transform transition-transform duration-300 ${
                        ...
                        {/* Header */}
                        <div className="p-4 lg:p-6 border-b border-gray-100 bg-gradient-to-r from-green-500 to-emerald-600">
                            ...
                            {/* Search */}
                            <div className="relative">
                                <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 h-4 w-4 lg:h-5 lg:w-5 text-white/70" />
                                <input 
                                    type="text" 
                                    placeholder="Search conversations..." 
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    className="w-full pl-10 lg:pl-12 pr-4 py-2 lg:py-3 bg-white/20 backdrop-blur-sm border border-white/30 rounded-xl focus:outline-none focus:ring-2 focus:ring-white/50 text-white placeholder-white/70 text-sm" 
                                />
                            </div>
                        </div>

                        {/* Filters */}
                        <div className="flex space-x-2 p-3 lg:p-4 border-b border-gray-100 overflow-x-auto scrollbar-hide">
                            {['all', 'unread', 'applicants', 'archived'].map(filter => (
                                <button 
                                    key={filter}
                                    onClick={() => setActiveFilter(filter)}
                                    className={`px-3 py-1.5 lg:px-4 lg:py-2 text-xs lg:text-sm font-semibold rounded-full whitespace-nowrap transition-all ${
                                        activeFilter === filter 
                                            ? 'bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-md' 
                                            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                                    }`}
                        ...
                        {/* Conversations List */}
                        <div className="flex-1 overflow-y-auto p-3 lg:p-4 space-y-2 lg:space-y-3 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
                            {isLoadingConversations ? (
                                <div className="flex justify-center items-center h-full">
                                    <Loader2 className="w-8 h-8 animate-spin text-green-500" />
                                </div>
                    ...
                    {/* Chat Area */}
                    <div className={`flex-1 flex flex-col bg-white ${selectedConversationId ? 'block' : 'hidden lg:block'}`}>
                        {selectedConversationId && conversationDetails ? (
                            <>
                                {/* Chat Header */}
                                <div className="p-4 lg:p-6 bg-white border-b border-gray-100 flex justify-between items-center flex-shrink-0 shadow-sm">
                                    <div className="flex items-center space-x-3 lg:space-x-4">
                                        ...
                                        <div className="relative">
                                            <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-gradient-to-br from-green-400 to-emerald-600 text-white flex items-center justify-center font-bold text-base lg:text-lg shadow-md">
                                                {activeChatInitials}
                                            </div>
                                            <div className="absolute bottom-0 right-0 w-2.5 h-2.5 lg:w-3.5 lg:h-3.5 bg-green-500 rounded-full border-2 border-white"></div>
                                        </div>
                                        <div>
                                            <p className="font-bold text-gray-900 text-base lg:text-lg">{activeChatName}</p>
                                            <p className="text-xs lg:text-sm text-green-600 flex items-center font-medium capitalize">
                                                <span className="w-1.5 h-1.5 lg:w-2 lg:h-2 bg-green-500 rounded-full mr-1 lg:mr-2 animate-pulse"></span>
                                                {activeChatRole}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex space-x-1 lg:space-x-2">
                                        <button className="hidden sm:flex px-3 lg:px-5 py-1.5 lg:py-2.5 text-xs lg:text-sm font-semibold text-green-600 border-2 border-green-600 rounded-xl hover:bg-green-50 transition-all">
                                            View Profile
                                        </button>
                                        <button className="p-2 lg:p-3 text-gray-500 hover:text-green-600 hover:bg-green-50 rounded-xl transition-all">
                                            <Phone className="w-4 h-4 lg:w-5 lg:h-5" />
                                        </button>
                                        <button className="p-2 lg:p-3 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-xl transition-all">
                                ...
                                {/* Messages */}
                                <div className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-1 bg-gradient-to-br from-gray-50 to-white scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
                                    {isLoadingMessages ? (
                                        <div className="flex justify-center items-center h-full">
                                            <Loader2 className="w-8 h-8 animate-spin text-green-500" />
                                        </div>
                                                ...
                                                <React.Fragment key={msg._id || index}>
                                                    {dateSeparator && (
                                                        <div className="flex justify-center my-4 lg:my-6">
                                                            <span className="text-xs font-semibold text-gray-500 bg-gray-200 px-3 py-1 lg:px-4 lg:py-2 rounded-full shadow-sm">
                                                                {dateSeparator}
                                                            </span>
                                                        </div>
                                                    )}
                                        ...
                                        <div className="flex flex-col items-center justify-center h-full text-gray-500">
                                            <div className="w-16 h-16 lg:w-24 lg:h-24 bg-gradient-to-br from-green-100 to-emerald-100 rounded-full flex items-center justify-center mb-4">
                                                <Send className="w-8 h-8 lg:w-12 lg:h-12 text-green-500" />
                                            </div>
                                            <p className="text-base lg:text-lg font-semibold">No messages yet</p>
                                            <p className="text-xs lg:text-sm mt-2">Send your first message below</p>
                                        </div>
                                    )}
                                    <div ref={messagesEndRef} />
                                </div>
                                ...
                                {/* Message Input */}
                                <form onSubmit={handleSendMessage} className="p-3 lg:p-6 bg-white border-t border-gray-100 flex space-x-2 lg:space-x-3 items-end flex-shrink-0 shadow-lg">
                                    <button type="button" className="p-2 lg:p-3 text-gray-500 hover:text-green-600 hover:bg-green-50 rounded-xl transition-all">
                                        <Paperclip className="w-4 h-4 lg:w-6 lg:h-6" />
                                    </button>
                                    <button type="button" className="p-2 lg:p-3 text-gray-500 hover:text-green-600 hover:bg-green-50 rounded-xl transition-all">
                                        <ImageIcon className="w-4 h-4 lg:w-6 lg:h-6" />
                                    </button>
                                    <div className="flex-1 relative">
                                        <input
                                            type="text"
                                            ...
                                            value={newMessage}
                                            onChange={(e) => setNewMessage(e.target.value)}
                                            className="w-full border-2 border-gray-200 focus:border-green-500 focus:ring-0 text-gray-800 px-3 lg:px-5 py-2 lg:py-3.5 bg-gray-50 rounded-2xl focus:bg-white transition-all text-sm lg:text-[15px]"
                                            disabled={isSending}
                                        />
                                    </div>
                                    <button 
                                        type="submit" 
                                        className="p-3 lg:p-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-2xl hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center" 
                                        disabled={!newMessage.trim() || isSending}
                                    >
                                        {isSending ? <Loader2 className="w-4 h-4 lg:w-5 lg:h-5 animate-spin" /> : <Send className="w-4 h-4 lg:w-5 lg:h-5" />}
                                    </button>
                                </form>
                            </>
                        ) : (
                            <div className="flex-1 flex flex-col items-center justify-center text-gray-500 bg-gradient-to-br from-gray-50 to-white">
                                {isLoadingConversations ? (
                                    ...
                                    <>
                                        <div className="w-20 h-20 lg:w-32 lg:h-32 bg-gradient-to-br from-green-100 to-emerald-100 rounded-full flex items-center justify-center mb-6">
                                            <Send className="w-10 h-10 lg:w-16 lg:h-16 text-green-500" />
                                        </div>
                                        <p className="text-lg lg:text-xl font-bold text-gray-700">No conversation selected</p>
                                        <p className="text-xs lg:text-sm text-gray-500 mt-2">Choose a conversation from the list to start chatting</p>
                                    </>
                                )}
                            </div>
                        )}
