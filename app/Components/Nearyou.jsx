...
import { Heart, MapPin, Wifi, Users, Calendar, ArrowDown, Loader2, ArrowLeft } from 'lucide-react';
...
const PropertyCard = ({ property, onApplyClick, isFavorited, onToggleFavorite }) => {
  const { title, location, price, features, tags, imageSrc, currency, distance, id } = property;
      ...
      <div className="flex-grow flex flex-col justify-between py-1">
        <div>
          ...
          <div className="flex items-center text-sm text-gray-500 mb-3">
            ...
            {location}
            {distance && <span className="ml-2 text-xs text-orange-600 font-medium">• {distance}</span>}
          </div>
...
const NearYouComponent = () => {
  ...
  const fetchProperties = async () => {
    ...
    try {
      ...
      console.log('🔍 Fetching properties near you...');
      const response = await fetch(`/api/listings/near-you?${params}`, {
        method: 'GET',
        headers: {
      ...
      if (data.success) {
        ...
        checkFavoriteStatus(data.properties.map(p => p.id));
      } else {
  ...
  }, [authUser, authLoading, router, sortBy, maxDistance, currentPage]);
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 p-4 md:p-6 lg:p-8 xl:p-10 overflow-y-auto">
          ...
          <div className="mb-4 md:mb-6 p-4 md:p-6 rounded-xl shadow-lg border border-gray-100 bg-white">
            ...
            {userLocation && (
              <>
                <p className="text-xs md:text-sm text-gray-600 mb-1 md:mb-2">{userLocation}</p>
          ...
          {!loading && !error && properties.length === 0 && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
              ...
              <p className="text-sm text-gray-600">
                Try adjusting your search radius or check back later for new listings.
              </p>
            </div>
          )}
