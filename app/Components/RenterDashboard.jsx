...
//   return (
//     <div
//       className={`p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between bg-white ${
...
//       {/* ✅ Use the new RenterHeader instead of hardcoded Header */}
...
//           <MultiStepApplicationForm onClose={() => setIsFormOpen(false)} />
//         </Modal>
//       )}
//     </div>
//   );
...
const ApplicationFormView = ({ onClose }) => (
  <div className="flex-1 p-8 bg-gray-50 overflow-y-auto mt-0">

    <div className="rounded-lg shadow-sm  border-gray-200 p-6">
      <MultiStepApplicationForm onClose={onClose} />
    </div>
  </div>
);

const RenterDashboard = () => {
  const [currentView, setCurrentView] = useState("dashboard"); // "dashboard" or "application"
  const { user, loading } = useAuth();
  const router = useRouter();
    ...
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="flex flex-1 overflow-hidden">
        {currentView === "dashboard" ? (
          ...
          <ApplicationFormView onClose={handleCloseApplication} />
        )}
      </div>
    </div>
  );
